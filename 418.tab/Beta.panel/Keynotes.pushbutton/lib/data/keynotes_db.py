# -*- coding: utf-8 -*-
"""Gestion des keynotes via DeffrelDB.

Portage depuis exemple/Keynotes.pushbutton/keynotesdb.py.
Sans système i18n (noms/labels gérés par la UI).
"""
...
class RKeynoteRegexFilter(RKeynoteFilter):
    def __init__(self, name='Regular Expression (Regex)', regex=None, negate=False):
        super(RKeynoteRegexFilter, self).__init__(
            name=('{}{}').format(name, ' [Exclude]' if negate else ''),
...
class RKeynoteFilters(object):
    ...
    UseCSINegate = RKeynoteRegexFilter(name='CSI MasterFormat Division No.', regex=CSI_REGEX, negate=True)
...
class RKeynote(object):
    ...
    @property
    def children(self):
        return self._filtered_children if self._filter else self._children
    ...
    def filter(self, search_term):
        ...
        if cleaned_sfilter:
            sterm = (self.key + ' ' + self.text + ' ' + self.owner).lower()
            ...
            if use_regex or use_regex_not:
                ...
                self_pass_keyword = coreutils.fuzzy_search_ratio(sterm, cleaned_sfilter) > 80
                self_pass = (self_pass_keyword and self_pass) if has_smart_filter else self_pass_keyword

        self._filtered_children = [x for x in self._children if x.filter(self._filter)]
        return bool(self_pass or self._filtered_children)
...
def get_categories(conn):
    ...
    locked_records = {x.LockTargetRecordKey: x.LockRequester for x in db_locks if x.IsRecordLock}
...
def get_keynotes_tree(conn):
    ...
    for catroot_rkey in keynote_records:
        if catroot_rkey.key in parents:
            catroot_rkey.children.extend(natsorted(parents[catroot_rkey.key], key=lambda x: x.key))
...
def update_category_title(conn, key, new_title):
    conn.UpdateRecord(KEYNOTES_DB, CATEGORIES_TABLE, key, {CATEGORY_TITLE_FIELD: new_title})
...
def move_keynote(conn, key, new_parent):
    conn.UpdateRecord(KEYNOTES_DB, KEYNOTES_TABLE, key, {KEYNOTES_PARENTKEY_FIELD: new_parent})
